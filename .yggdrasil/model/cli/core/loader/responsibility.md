...
- **scanModelDirectory**: Internal recursive scan; skips dirs without yg-node.yaml; reads artifacts via readArtifacts (excludes yg-node.yaml, includes only config.artifacts keys); skips dirs starting with '.'.

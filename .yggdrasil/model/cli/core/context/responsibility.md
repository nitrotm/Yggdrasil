...
- **6-step build, 5-section render**: Build phase: (1) global config, (2) hierarchy ancestors, (3) own (node.yaml from disk + configured artifacts from node), (4) relational (structural_context per decisions/002 or fallback), (5) flows (node + ancestors), (6) aspects (union of tags from hierarchy + own + flow layers; no source on aspect). Render order differs: Global → Hierarchy → OwnArtifacts → Aspects → Relational (deps+flows merged).

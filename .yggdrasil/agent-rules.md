...
- [ ] 3. Create node directory, read `schemas/yg-yg-node.yaml`, create `yg-yg-node.yaml`
- [ ] 4. Analyze source — for each artifact type in `yg-yg-config.yaml artifacts`: extract content, do not invent
- [ ] 5. Identify relations — add to `yg-yg-node.yaml`
- [ ] 6. Identify cross-cutting requirements — add matching aspects, create if needed
- [ ] 6b. For each aspect on the node: identify 2-5 code anchors (function names, constants) that evidence the pattern → add as `anchors` in the aspect entry in `yg-yg-node.yaml`
...
- [ ] 4. For each aspect: verify the pattern holds in THIS node. If it deviates, add `exceptions` to the aspect entry in `yg-yg-node.yaml`
...
```
.yggdrasil/
  yg-yg-config.yaml     ← vocabulary, node types, artifact rules, required aspects
...
- **Aspect id = directory path** under `aspects/`. Each aspect has `yg-yg-aspect.yaml` + content `.md` files. No automatic parent-child — use `implies` explicitly.
...
Projects can define additional artifact types in `yg-yg-config.yaml` under `artifacts`. Each custom artifact has a `description` (tells you what to write), a `required` condition (`always`, `never`, `when: has_incoming_relations`, `when: has_aspect:<id>`), and an `included_in_relations` flag (if true, included in dependency context packages for structural relations). The three standard artifacts are always present in config. Check `yg-yg-config.yaml` to see all defined artifacts for the project.
...
- **Specific to this node** → local node artifact (check `yg-yg-config.yaml artifacts` for available types)
- **Rule for many nodes** → aspect (`aspects/<id>/` with `yg-yg-aspect.yaml` + content `.md` files). If applies to ALL nodes of a type → `node_types.<type>.required_aspects` in `yg-yg-config.yaml`
- **Business process** → flow (`flows/<name>/` with `yg-yg-flow.yaml` + `description.md`). Ask user if process unclear.
...
- [ ] 1. Read `schemas/yg-yg-aspect.yaml`
- [ ] 2. Create `aspects/<id>/` directory
- [ ] 3. Write `yg-yg-aspect.yaml` — name, optional description, optional implies
...
**Aspect stability tiers.** If an aspect has a `stability` field in `yg-yg-aspect.yaml`, use it to calibrate review urgency:
...
When code anchors (`anchors` in an aspect entry in `yg-yg-node.yaml`) are present, they list code patterns (function names, constants, SQL fragments) evidencing the aspect's implementation in this node. `yg validate` checks that each anchor exists in the node's mapped source files — a missing anchor (W014) signals the aspect may be stale for this node.

### Creating Flows

- [ ] 1. Read `schemas/yg-yg-flow.yaml`
- [ ] 2. Create `flows/<name>/` directory
- [ ] 3. Write `yg-yg-flow.yaml` — declare participants and flow-level aspects
...
- **Read schemas before creating** any `yg-yg-node.yaml`, `yg-yg-aspect.yaml`, or `yg-yg-flow.yaml`.
...
|---|---|
| Information specific to this node | Local node artifact (check `yg-yg-config.yaml artifacts` for types) |
| Rule that applies to many nodes | Aspect (content `.md` files in `aspects/<id>/`) |
| Architectural invariant for a node type | Required aspect in `yg-yg-config.yaml node_types` |
| Business process participation | Flow (`yg-yg-flow.yaml participants`) |

...
4. \`yg validate\` -> Fix any structural errors.
...
**Reverse engineering order:** When reverse-engineering an area, create graph elements in this order: (1) aspects, (2) flows, (3) model nodes. Never create model nodes before cross-cutting rules exist — they depend on them.
...
2. **Read Config & Templates:**
   ...
   * **CRITICAL:** ALWAYS read the schema in \`.yggdrasil/templates/\` for the element type (node.yaml, aspect.yaml, flow.yaml) before creating or editing it.
...
- [ ] 1. Read schema from \`.yggdrasil/templates/\` (node.yaml, aspect.yaml, or flow.yaml for the element type)
...
* **\`.yggdrasil/config.yaml\`**: Defines \`node_types\`, \`tags\`, \`artifacts\`.
* **\`.yggdrasil/templates/\`**: Schemas for each graph layer — \`node.yaml\`, \`aspect.yaml\`, \`flow.yaml\`.
...
## 7. CONTEXT ASSEMBLY (HOW TO MAP FILES)
...
However, you must NOT dump all context into a single file. Yggdrasil's context package is **multi-layered** and hierarchically assembled. When you map existing code or design new code, you must deconstruct content and place it at the correct abstraction layer so the engine can mechanically reassemble it.
...
2. **NEVER Invent the "Why":** Artifacts that imply human judgment (e.g. local decisions) must reflect ACTUAL human choices.
...
* **Routing:** **DO NOT ASSUME FILE NAMES.** You MUST read \`.yggdrasil/config.yaml\` (the \`artifacts\` section) to see the exact allowed filenames for the current project and their requirement conditions (e.g., \`required: always\` vs \`when: has_incoming_relations\`). Write node-specific content ONLY into these configured files next to \`node.yaml\`.
...
- If yes, but the local files are bloated -> You failed to deconstruct into Tags, Aspects, Flows, and Hierarchy. Fix the routing.
